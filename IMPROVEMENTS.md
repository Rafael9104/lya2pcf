...
- **#20** — Streaming GPU upload: host RAM per rank no longer grows with the
  slice (implemented on `feat/streaming-upload`, measured 2026-09-19)
...
- `two_point.py` and `distortion.py` use it on the GPU path
  (`init(..., plan=plan)`); the `--cpu` path keeps the old loader, since it
  needs every array. The old `init(data)` path still works.
